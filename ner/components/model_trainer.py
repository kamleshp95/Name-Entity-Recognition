...
from ner.entity.artifact_entity import (DataTransformationArtifacts,ModelTrainingArtifacts)
...
class ModelTraining:
    ...
    def initiate_model_training(self) -> ModelTrainingArtifacts:
        try:
            ...
            logging.info(f"Created {os.path.basename(self.model_trainer_config.model_training_artifacts_dir)} directory.")
            ...
            for epoch_num in range(epochs):
                ...
                print(f"Epochs: {epoch_num + 1} | Loss: {total_loss_train / len(df_train): .3f} | Accuracy: {total_acc_train / len(df_train): .3f} | Val_Loss: {val_loss: .3f} | Accuracy: {val_accuracy: .3f}")
            ...
            logging.info(f"Uploaded pickle file to the Google storage. File name - {os.path.basename(self.model_trainer_config.tokenizer_file_path)}")
